...
import {
  Trophy, ClipboardList, Calendar, User, Star, X, Check,
  Clock, ChevronRight, Upload, Eye, EyeOff, Bell,
  Loader2, Camera, Lock, CalendarClock,
...
function NotSetUpScreen() {
  return (
    <div className="min-h-screen w-full flex items-center justify-center text-white p-6 text-center relative" style={{ background: BG_GRADIENT, fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
      <GlobalStyleTag />
      <BackgroundBlobs />
      <div className={`${GLASS} rounded-3xl p-8 max-w-sm relative z-10`}>
        ...
        <p className="text-white/50 text-sm">O'qituvchingiz hali hisobini yaratmagan. Birozdan so'ng qayta urinib ko'ring yoki o'qituvchingizga murojaat qiling.</p>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen w-full flex items-center justify-center text-white p-4 relative" style={{ background: BG_GRADIENT, fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <div className={`${GLASS} rounded-3xl p-6 sm:p-8 w-full max-w-sm relative z-10`} style={{ background: 'rgba(10,20,60,0.8)' }}>
        <div className="text-center mb-6">
          ...
          <h1 className="font-display text-xl font-bold text-white">O'quvchi Panel</h1>
        ...
        <div className="space-y-3">
          ...
          <p className="text-white/30 text-[11px] text-center pt-1">Hisobingiz yo'qmi? O'qituvchingizdan so'rang.</p>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (hasStorage) {
        ...
        try {
          const sessRes = await window.storage.get(STUDENT_SESSION_KEY, false).catch(() => null);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!hasStorage) return;
      try {
        if (session) await window.storage.set(STUDENT_SESSION_KEY, JSON.stringify(session), false);
        else await window.storage.delete(STUDENT_SESSION_KEY, false);
      } catch (e) { /* ignore */ }
    })();
  }, [session, loading]);
  ...
  function loginAsStudent(studentId) { setSession({ studentId }); setView('home'); }
  function logout() { setSession(null); setView('home'); }
  ...
  function markSubmission(taskId, studentId, data) {
    ...
    if (data.status === 'submitted') addNotification(`"${task.title}" topshirildi.`);
  }
  ...
  if (!session) return <StudentLoginScreen appData={appData} onLoginStudent={loginAsStudent} />;
