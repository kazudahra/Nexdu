...
import {
  ...
  persistDirectorData,
  persistOpData,
  persistDirectorSession,
  persistDirectorNotifLog,
} from "./director_backend";
...
const INPUT_CLS =
  "w-full bg-white/10 backdrop-blur-md border border-white/20 rounded-xl px-4 py-2.5 text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-white/40 focus:bg-white/15 transition-all text-sm";
const LABEL_CLS =
  "block text-xs font-medium text-white/60 mb-1.5 uppercase tracking-wide";
const BTN_PRIMARY_BASE =
  "backdrop-blur-md border border-white/30 text-white font-medium rounded-xl px-4 py-2.5 transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 text-sm disabled:opacity-40 disabled:cursor-not-allowed disabled:active:scale-100 hover:brightness-110";
const BTN_GHOST =
...
function Avatar({ name, color = "#8b5cf6", size = 40, photo, onClick }) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={`font-display rounded-full flex items-center justify-center font-bold text-white border-2 border-white/30 shrink-0 ${onClick ? "cursor-pointer" : ""}`}
    >
      {initials(name)}
    </div>
  );
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "rgba(10,20,60,0.92)" }}
      >
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "rgba(10,20,60,0.95)" }}
      >
...
function StatCard({ label, value, sub, icon: Icon }) {
  return (
    <div className={`${GLASS} rounded-2xl p-4`}>
      ...
      <p className="font-display text-white text-xl font-bold truncate">
        {value}
      </p>
      ...
      {[1, 2, 3, 4, 5].map((s) => (
        <button key={s} type="button" onClick={() => onChange(s)}>
          <Star
            size={size}
            className={
              s <= value ? "fill-amber-300 text-amber-300" : "text-white/25"
            }
          />
        </button>
      ))}
    ...
    <div className="flex flex-wrap gap-1.5">
      {WEEK_DAYS.map((d) => (
        <button
          ...
          type="button"
          onClick={() => toggle(d)}
          className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all ${value.includes(d) ? "bg-white/25 border-white/40 text-white" : "bg-white/5 border-white/10 text-white/50"}`}
        >
          {d}
        </button>
      ))}
    ...
    <div className="flex flex-wrap gap-1.5">
      {branches.map((b) => (
        <button
          ...
          type="button"
          onClick={() => toggle(b.id)}
          className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all ${value.includes(b.id) ? "bg-white/25 border-white/40 text-white" : "bg-white/5 border-white/10 text-white/50"}`}
        >
          {b.name}
        </button>
      ))}
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <button
        ...
        className={`w-11 h-6 rounded-full transition-all relative shrink-0 ${checked ? "" : "bg-white/10 border border-white/20"}`}
          ...
          >
            {allThemes.map((t) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all ${theme.id === t.id ? "bg-white/20 text-white" : "text-white/70 hover:bg-white/10"}`}
              >
                <span
...
function TwoFactorStep({ demoCode, onVerify, onBack, onResend }) {
  ...
  function verify() {
    if (code === demoCode) onVerify();
    else setError("Kod noto'g'ri.");
  }
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-white/70 text-sm mb-1">
        <ShieldCheck size={16} /> Ikki bosqichli tekshiruv
      </div>
      <p className="text-white/50 text-xs">
        Telefoningizga yuborilgan 5 xonali kodni kiriting.
      </p>
      <div
        className={`${GLASS_SOFT} rounded-xl px-3 py-2 text-center text-white/60 text-xs`}
      >
        Demo kod:{" "}
      ...
      <input
        ...
        placeholder="00000"
        inputMode="numeric"
        className={`${INPUT_CLS} text-center tracking-[0.5em] text-lg`}
        autoFocus
      ...
      <PrimaryButton onClick={verify} className="w-full">
        <ShieldCheck size={16} /> Tasdiqlash
      </PrimaryButton>
    </div>
  );
}
...
function DirectorAuth({ directorData, onLoginDirector, onRegister }) {
  ...
  async function handleLogin() {
    ...
    const dir = directorData.directors.find(
      (d) => normalizePhone(d.phone) === normalized && d.passwordHash === hash,
    );
    setBusy(false);
    if (!dir) {
    ...
    if (dir.twoFactorEnabled) {
      setTwoFAStep({ directorId: dir.id, demoCode: generateDemoCode() });
      return;
    }
  ...
  async function handleRegister() {
    ...
    if (
      directorData.directors.some((d) => normalizePhone(d.phone) === normalized)
    ) {
      setError("Bu telefon raqam bilan direktor allaqachon ro'yxatdan o'tgan.");
      return;
    }
    setBusy(true);
  ...
  async function handleLogoUpload(e) {
    ...
    try {
      ...
      setRegLogo(await compressImageDataUrl(raw, 240));
    } catch (err) {
      console.error(err);
    }
  }
  ...
  return (
    <div
      className="min-h-screen w-full text-white relative flex"
      style={{
        background: theme.bg,
    ...
    >
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-center px-16 relative z-10">
        ...
        <p className="text-white/60 text-base max-w-md">
          Filiallar, menejerlar, o'qituvchilar, kurslar, to'lovlar va moliya —
          barchasi bitta direktor panelida.
        </p>
      </div>
      <div className="flex-1 flex items-center justify-center p-4 relative z-10">
        ...
        >
          <div className="text-center mb-6 lg:hidden">
            ...
            <h1 className="font-display text-xl font-bold">Direktor Panel</h1>
          </div>
          {twoFAStep ? (
            <TwoFactorStep
              demoCode={twoFAStep.demoCode}
              onVerify={() => onLoginDirector(twoFAStep.directorId)}
              onBack={() => setTwoFAStep(null)}
              onResend={() =>
                setTwoFAStep((s) => ({ ...s, demoCode: generateDemoCode() }))
              }
            />
          ) : (
            <>
              <div className="flex gap-2 mb-5 bg-white/5 border border-white/10 rounded-2xl p-1">
                <button
                  ...
                  className={`flex-1 text-sm py-2 rounded-xl transition-all ${mode === "login" ? "bg-white/20 text-white" : "text-white/50"}`}
                >
                  Kirish
                </button>
                <button
                  ...
                  className={`flex-1 text-sm py-2 rounded-xl transition-all ${mode === "register" ? "bg-white/20 text-white" : "text-white/50"}`}
                >
                  Ro'yxatdan o'tish
                </button>
              </div>
              {mode === "login" ? (
                ...
                <div className="space-y-3">
                  <div>
                    <label className={LABEL_CLS}>To'liq ism</label>
                    <input
                      value={regName}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Masalan: Iqbol Ta'lim"
                      className={INPUT_CLS}
                    />
                  </div>
                  <div>
                    <label className={LABEL_CLS}>Yashash / manzil</label>
                    <input
                      value={regAddress}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        {allBranches && scopeBranches.length > 1 && (
          ...
          >
            <option value="all" className="bg-violet-950">
              Barcha filiallar
            </option>
            {scopeBranches.map((b) => (
              <option key={b.id} value={b.id} className="bg-violet-950">
                {b.name}
              </option>
            ))}
          </select>
      ...
      {pendingCount > 0 && (
        <div className={`${GLASS} rounded-2xl p-4 flex items-center gap-3`}>
          <Bell size={18} style={{ color: theme.accent1 }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-white font-semibold mb-4 flex items-center gap-2">
            <CreditCard size={16} /> To'lov turlari (bu oy)
          </h3>
        ...
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-white font-semibold mb-4 flex items-center gap-2">
            <GraduationCap size={16} /> O'qituvchilar bahosi
          </h3>
      ...
      <div className={`${GLASS} rounded-3xl p-5`}>
        <h3 className="font-display text-white font-semibold mb-4 flex items-center gap-2">
          <Sparkles size={16} className="text-amber-300" /> Kurslar bo'yicha
          o'quvchilar soni va daromad
        </h3>
      ...
      {branchCompareData.length > 0 && (
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-white font-semibold mb-4 flex items-center gap-2">
            <Building2 size={16} /> Filiallar bo'yicha sof foyda (bu oy)
          </h3>
          <div style={{ width: "100%", height: 220 }}>
            <ResponsiveContainer>
...
function BranchesPage({ director, directorData, opData, openModal }) {
  const myBranches = directorData.branches.filter(
    (b) => b.directorId === director.id,
  );
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-white/50 text-sm mt-0.5">
            {myBranches.length} ta filial
          </p>
        </div>
        <PrimaryButton onClick={() => openModal({ type: "branchForm" })}>
      ...
      {myBranches.length === 0 ? (
        <EmptyState
          ...
          subtitle="Birinchi filialingizni qo'shing."
        />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {myBranches.map((b) => {
            ...
            return (
              <div
                key={b.id}
                className={`${GLASS} rounded-3xl p-5 cursor-pointer hover:bg-white/15 transition-colors`}
                onClick={() =>
                  openModal({ type: "branchDetail", branchId: b.id })
                }
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      openModal({ type: "branchForm", editing: b });
                    }}
                    className={BTN_ICON}
                  >
                    <Pencil size={14} />
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-white/40 text-[10px]">Yig'ilgan</p>
                  </div>
                  <div>
                    <p className="text-rose-300 text-base font-bold">
                      {money(stats.expenses)}
                    </p>
                    <p className="text-white/40 text-[10px]">Xarajat</p>
                  </div>
                  <div>
                    <p className="text-emerald-300 text-base font-bold">
                      {money(stats.netProfit)}
                    </p>
                    <p className="text-white/40 text-[10px]">Sof foyda</p>
                  </div>
                </div>
                <p className="text-white/35 text-[11px] mt-2">
                  {stats.activeStudents} o'quvchi · {stats.teacherCount}{" "}
                  o'qituvchi · {stats.courseCount} kurs — to'liq hisobot uchun
                  bosing
                </p>
              </div>
            );
  ...
  return (
    <Modal title={branch.name} onClose={onClose} wide>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <StatCard
          icon={CheckCircle2}
          label="Yig'ilgan (bu oy)"
          value={money(stats.collected) + " so'm"}
        />
        <StatCard
          icon={TrendingDown}
          label="Xarajat (bu oy)"
          value={money(stats.expenses) + " so'm"}
        />
        <StatCard
          icon={TrendingUp}
          label="Sof foyda (bu oy)"
          value={money(stats.netProfit) + " so'm"}
        />
        <StatCard
          icon={Wallet}
          label="Kutilayotgan oylik"
          value={money(stats.expectedRevenue) + " so'm"}
        />
        <StatCard
          icon={Users}
          label="Faol o'quvchi"
...
function ManagersPage({ director, directorData, openModal }) {
  ...
  const managers = directorData.managers
    .filter((m) => (m.branchIds || []).some((id) => myBranchIds.includes(id)))
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-white/50 text-sm mt-0.5">
            {managers.length} ta menejer · menejerlar alohida "Menejer" ilovasi
            orqali shu telefon/parol bilan kiradi
          </p>
        </div>
        <PrimaryButton
          onClick={() => openModal({ type: "managerForm" })}
      ...
      {best && best.rating >= 4 && (
        <div className={`${GLASS} rounded-3xl p-5 flex items-center gap-3`}>
          ...
          <div>
            <p className="text-white/50 text-xs">Eng yaxshi ishlayotgan</p>
      ...
      {managers.length === 0 ? (
        <EmptyState
          icon={Users}
          title="Hali menejer yo'q"
          subtitle="Avval filial yarating, keyin menejer qo'shing."
        />
      ) : (
            ...
            return (
              ...
              >
                ...
                <p className="text-white text-sm font-semibold shrink-0">
                  {money(m.monthlySalary)} so'm
                </p>
                <button
                  onClick={() => openModal({ type: "managerForm", editing: m })}
                  className={BTN_ICON}
                  title="Tahrirlash"
                >
                  <Pencil size={14} />
    ...
    <Modal title={`${manager.name} — ruxsatlar`} onClose={onClose}>
      <div className="space-y-2">
        ...
        {MANAGER_NAV_ALL.map((p) => (
          <label
            key={p.id}
            className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-3 cursor-pointer"
          >
            <input
              type="checkbox"
            ...
            />
            <p.icon size={16} className="text-white/60" />
    ...
    <div className="space-y-5">
      ...
      {best && (
        <div className={`${GLASS} rounded-3xl p-5 flex items-center gap-3`}>
          <span className="text-2xl">🏆</span>
          <div>
            <p className="text-white/50 text-xs">Eng yuqori baholangan</p>
            <p className="font-display text-white font-semibold">
              {best.name} — {best.rating}/5
            </p>
          </div>
              ...
              <div key={t.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                ...
                <div className="grid grid-cols-4 gap-2 pt-2 border-t border-white/10">
                  <div>
                    ...
                    <p className="text-white/40 text-[10px]">Kelishuv</p>
                  </div>
                  <div>
                    <p className="text-white text-sm font-bold">
                      {activeStudents}
                    </p>
                    <p className="text-white/40 text-[10px]">Faol o'quvchi</p>
                  </div>
                  <div>
                    <p className="text-emerald-300 text-sm font-bold">
                      {money(stats.expectedPay)}
                    </p>
                    <p className="text-white/40 text-[10px]">Oylik haqi</p>
                  </div>
                  <div>
                    <p className="text-amber-300 text-sm font-bold">
                      {money(stats.remaining)}
                    </p>
                    <p className="text-white/40 text-[10px]">Qolgan haq</p>
                  </div>
                </div>
...
function HolidaysPage({
  ...
  return (
    <div className="space-y-5">
      ...
      {holidays.length === 0 ? (
        <EmptyState icon={PartyPopper} title="Hali bayram kuni yo'q" />
      ) : (
        <div className="space-y-2">
          {holidays.map((h) => (
            <div
            ...
            >
              <div className="flex items-center gap-3">
                <PartyPopper size={18} className="text-amber-300 shrink-0" />
                <div>
        ...
        <div className="flex gap-1.5 bg-white/5 border border-white/10 rounded-xl p-1">
          {[
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg transition-all ${filter === id ? "bg-white/20 text-white" : "text-white/50"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="space-y-2">
          {filtered.map((n) => (
            <button
              ...
              className={`${GLASS} w-full text-left rounded-2xl p-4 flex items-start gap-3 transition-all ${n.read ? "opacity-60" : "hover:bg-white/10"}`}
            >
              <span
                className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.read ? "bg-white/20" : "bg-sky-400"}`}
              />
...
function FinancePage({
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="font-display text-2xl font-bold text-white">Moliya</h2>
          <p className="text-white/50 text-sm mt-0.5">
            ...
            <div>
              ...
              >
                {EXPENSE_CATEGORIES.map((c) => (
                  <option key={c} value={c} className="bg-violet-950">
                    {c}
                  </option>
                ))}
              </select>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 text-xs py-2 rounded-xl border transition-all ${approvalMode === "manager" ? "bg-white/20 border-white/40 text-white" : "bg-white/5 border-white/10 text-white/50"}`}
              >
                Menejer ruhsati (darhol)
              </button>
              <button
                onClick={() => setApprovalMode("director")}
                className={`flex-1 text-xs py-2 rounded-xl border transition-all ${approvalMode === "director" ? "bg-white/20 border-white/40 text-white" : "bg-white/5 border-white/10 text-white/50"}`}
              >
      ...
      {role === "director" && pending.length > 0 && (
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-white font-semibold mb-3 flex items-center gap-2">
            <Bell size={16} /> Tasdiqlash kutilmoqda ({pending.length})
          </h3>
          <div className="space-y-2">
            {pending.map((f) => {
              ...
              return (
                <div
                  key={f.id}
                  className="flex items-center justify-between gap-3 bg-white/5 border border-white/10 rounded-2xl p-3.5"
                >
                  <div className="min-w-0">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button
                      ...
                      className="w-9 h-9 rounded-xl bg-emerald-400/20 hover:bg-emerald-400/30 border border-emerald-300/40 flex items-center justify-center text-emerald-200"
                    >
                      <CheckCircle2 size={16} />
                    </button>
                    <button
                      onClick={() => rejectFinance(f.id)}
                      className="w-9 h-9 rounded-xl bg-rose-400/20 hover:bg-rose-400/30 border border-rose-300/40 flex items-center justify-center text-rose-200"
                    >
                      <XCircle size={16} />
                    </button>
                  </div>
      ...
      <div className={`${GLASS} rounded-3xl p-5`}>
        ...
        {history.length === 0 ? (
          <p className="text-white/40 text-sm">Hali yozuv yo'q.</p>
        ) : (
          <div className="space-y-2">
            {history.map((f) => (
              <div
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-white/40 text-xs">{formatDate(f.date)}</p>
                </div>
                <p
                  className={`text-sm font-semibold shrink-0 ${f.type === "income" ? "text-emerald-300" : "text-rose-300"}`}
                >
                  {f.type === "income" ? "+" : "-"}
                  {money(f.amount)}
                </p>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex gap-1.5 mt-1 flex-wrap">
                    {status === "paid" && (
                      <span className="text-[10px] bg-emerald-400/20 border border-emerald-300/40 text-emerald-200 px-2 py-0.5 rounded-full">
                        To'landi
                      </span>
                    )}
                    {status === "partial" && (
                      <span className="text-[10px] bg-amber-400/20 border border-amber-300/40 text-amber-200 px-2 py-0.5 rounded-full">
                        Qisman: {money(paidThis)}/{money(price)}
                      </span>
                    )}
                    {status === "unpaid" && (
                      <span className="text-[10px] bg-rose-400/20 border border-rose-300/40 text-rose-200 px-2 py-0.5 rounded-full">
                        To'lanmagan
                      </span>
                    )}
                    {hasDebt && (
                      <span className="text-[10px] bg-rose-500/30 border border-rose-400/50 text-rose-100 px-2 py-0.5 rounded-full">
                        O'tgan oydan qarz
                      </span>
                    )}
                    {overdue && (
                      <span className="text-[10px] bg-red-500/40 border border-red-400/60 text-white px-2 py-0.5 rounded-full flex items-center gap-1">
                        <AlertTriangle size={10} /> Muddati o'tgan
                      </span>
                    )}
                  </div>
                </div>
  ...
  return (
    <Modal title="To'lov qabul qilish" onClose={onClose}>
      <div className="space-y-4">
            ...
            <div className="mt-1.5 space-y-1 max-h-40 overflow-y-auto">
              {matches.map((s) => (
                <button
                  ...
                  className="w-full flex items-center gap-2 bg-white/5 hover:bg-white/15 border border-white/10 rounded-xl p-2 text-left transition-colors"
                >
        ...
        {selectedStudent && studentGroupOptions.length > 0 && (
          <div>
            ...
            <select
              ...
              className={INPUT_CLS}
            >
              <option value="" className="bg-violet-950">
                — Tanlang —
              </option>
          ...
          <div className="flex gap-2">
            {PAYMENT_METHODS.map((m) => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 text-sm py-2.5 rounded-xl border transition-all ${method === m.id ? "bg-white/20 border-white/40 text-white" : "bg-white/5 border-white/10 text-white/50"}`}
              >
        ...
        {error && <p className="text-rose-300 text-xs">{error}</p>}
        <PrimaryButton onClick={submit} className="w-full">
          <Check size={16} /> To'lovni saqlash
        </PrimaryButton>
      </div>
    </Modal>
  );
                  ...
                  <div className="px-5 pb-5 border-t border-white/10 pt-4 space-y-2">
                    ...
                    {canEdit && (
                      <button
                        ...
                        className={`${BTN_GHOST} w-full mt-2 text-rose-200 hover:text-rose-100`}
                      >
                        <Trash2 size={14} /> Kursni o'chirish
                      </button>
                    )}
                  </div>
                )}
    ...
    try {
      ...
      if (avg)
        updateDirector({
          ...
          logo: compressed,
          themeId: "custom",
          customTheme: buildCustomTheme(avg.r, avg.g, avg.b),
        });
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      <div>
        ...
        <p className="text-white/50 text-sm mt-0.5">
          Ko'rinish, brend ranglari va xavfsizlik
        </p>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              ...
              <button
                key={t.id}
                onClick={() => updateDirector({ ...director, themeId: t.id })}
                className={`rounded-2xl overflow-hidden border-2 transition-all ${theme.id === t.id ? "border-white scale-105" : "border-white/10"}`}
              >
                <div className="h-14 w-full" style={{ background: t.bg }} />
                <p className="text-[11px] text-white/70 p-1.5 text-center truncate">
                  {t.name}
                </p>
              </button>
            ))}
          ...
          <p className="text-white/40 text-[11px] mt-2">
            Har qanday sahifada, yuqori panelda ham mavzuni tezkor almashtirish
            tugmasi bor.
          </p>
        </div>
      </div>
      <div className={`${GLASS} rounded-3xl p-6 space-y-3`}>
        <ToggleSwitch
          checked={director.twoFactorEnabled}
          onChange={(v) => updateDirector({ ...director, twoFactorEnabled: v })}
          label="Ikki bosqichli tekshiruv (2FA)"
          sub="Kirishda qo'shimcha 5 xonali kod so'raladi"
        />
        <p className="text-white/35 text-[11px]">
          Hozircha faqat dizayn/interfeys — SMS yuborish uchun keyinchalik
          backend ulanishi kerak bo'ladi.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 shrink-0 p-5 gap-1">
      <div className={`${GLASS} rounded-3xl p-4 mb-4`}>
        <p className="font-display text-white font-bold text-lg tracking-tight truncate">
          {title}
        </p>
      </div>
      <div
    ...
    >
      {items.map((item) => (
        <button
          key={item.id}
          onClick={() => goTo(item.id)}
          className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all shrink-0 ${view === item.id ? "bg-white/20 text-white" : "text-white/50"}`}
        >
          <item.icon size={17} />
...
function ManagerFormModal({ editing, branches, onSubmit, onClose }) {
  ...
  async function submit() {
    if (!name.trim() || !phone.trim() || branchIds.length === 0) {
      setError("Ism, telefon va kamida bitta filial majburiy.");
      return;
    }
    if (!editing && (!password || password.length < 4)) {
      setError("Yangi menejer uchun parol (kamida 4 belgi) kerak.");
      return;
    }
    setBusy(true);
    const payload = {
  ...
  return (
    <Modal
      title={editing ? "Menejerni tahrirlash" : "Menejer qo'shish"}
      onClose={onClose}
    >
      <div className="space-y-4">
        <div>
        ...
        <div>
          <label className={LABEL_CLS}>Yashash joyi</label>
          <input
            value={address}
            onChange={(e) => setAddress(e.target.value)}
            className={INPUT_CLS}
        ...
        <div>
          <label className={LABEL_CLS}>Ishlaydigan filiallari</label>
          <BranchPicker
            branches={branches}
            value={branchIds}
        ...
        <div>
          <label className={LABEL_CLS}>Ish samaradorligi</label>
          <StarPicker value={rating} onChange={setRating} />
        </div>
        <div>
          <label className={LABEL_CLS}>
            {editing ? "Yangi parol (bo'sh qoldirsa o'zgarmaydi)" : "Parol"}
          </label>
          <input
            type="text"
            value={password}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`text-left rounded-xl border p-3 transition-all ${salaryType === "percent" ? "bg-white/15 border-white/40" : "bg-white/5 border-white/10"}`}
            >
              <p className="text-white text-sm font-medium">To'lovdan foiz</p>
            ...
            <button
              ...
              className={`text-left rounded-xl border p-3 transition-all ${salaryType === "fixed" ? "bg-white/15 border-white/40" : "bg-white/5 border-white/10"}`}
            >
              <p className="text-white text-sm font-medium">
                Belgilangan oylik
              </p>
...
function TeacherPayrollModal({
  ...
  return (
    <Modal title={`${teacher.name} — maosh hisob-kitobi`} onClose={onClose}>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full text-white/70">
            {teacher.salaryType === "fixed"
              ? "Belgilangan oylik"
        ...
        >
          <div>
            <p className="text-white/40 text-[11px]">Oylik haqi</p>
            <p className="text-white font-semibold">
              {money(stats.expectedPay)} so'm
            </p>
          </div>
          <div>
            <p className="text-white/40 text-[11px]">Avans olgan</p>
            <p className="text-amber-300 font-semibold">
              {money(stats.advances)} so'm
            </p>
          </div>
          <div>
            <p className="text-white/40 text-[11px]">Maosh olgan</p>
            <p className="text-sky-300 font-semibold">
              {money(stats.salaryPaid)} so'm
            </p>
          </div>
          <div>
            <p className="text-white/40 text-[11px]">Qolgan haqi</p>
        ...
        <div className="space-y-3">
          <div className="flex gap-1.5 bg-white/5 border border-white/10 rounded-xl p-1">
            <button
              ...
              className={`flex-1 text-xs py-2 rounded-lg transition-all ${tab === "advance" ? "bg-amber-400/30 text-amber-100" : "text-white/50"}`}
            >
              Avans berish
            </button>
            <button
              onClick={() => setTab("salary")}
              className={`flex-1 text-xs py-2 rounded-lg transition-all ${tab === "salary" ? "bg-sky-400/30 text-sky-100" : "text-white/50"}`}
            >
              Maosh to'lash
            </button>
          </div>
        ...
        {allHistory.length > 0 && (
          <div className="border-t border-white/10 pt-3 space-y-1.5 max-h-48 overflow-y-auto">
            <p className="text-white/40 text-[11px] mb-1">To'lovlar tarixi</p>
            {allHistory.map((p) => (
              <div
                key={p.id}
              ...
              >
                <div>
                  <span
                    className={
                      p.type === "advance" ? "text-amber-300" : "text-sky-300"
                    }
                  >
      ...
      <div className="space-y-4">
        <div>
          ...
          >
            ...
            {courses.map((c) => (
              <option key={c.id} value={c.id} className="bg-violet-950">
                {c.name}
              </option>
            ))}
          </select>
        ...
        {rooms && rooms.length > 0 && (
          <div>
            ...
            >
              ...
              {rooms.map((r) => (
                <option key={r.id} value={r.id} className="bg-violet-950">
...
export default function App() {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setOpData(data.opData || { groups: [], students: [], tasks: [] });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await persistDirectorSession(session);
      } catch (e) {
        console.error("Saqlashda xatolik:", e);
      }
    })();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        await persistDirectorNotifLog(notifLog);
      } catch (e) {
        console.error("Saqlashda xatolik:", e);
      }
    }, 700);
  ...
  function loginAsDirector(directorId) {
    setSession({ directorId });
    setView("home");
  }
  function logout() {
    setSession(null);
  ...
  function addBranch(payload) {
    ...
    addNotification(`"${payload.name}" filiali qo'shildi.`);
  }
  function updateBranch(id, payload) {
    setDirectorData((prev) => ({
      ...prev,
    ...
    addNotification("Filial yangilandi.");
  }
  function addManager(payload) {
    ...
    addNotification("Menejer qo'shildi.");
  }
  function updateManager(id, payload) {
    setDirectorData((prev) => ({
      ...prev,
    ...
    addNotification("Menejer yangilandi.");
  }
  function deleteManager(managerId) {
    ...
    addNotification("Menejer o'chirildi.");
  }
  function updateManagerPermissions(managerId, allowedPages) {
    setDirectorData((prev) => ({
      ...prev,
      managers: prev.managers.map((m) =>
        m.id === managerId ? { ...m, allowedPages } : m,
      ),
    }));
    addNotification("Ruxsatlar yangilandi.");
  }
  ...
  const currentDirector = session
    ? directorData.directors.find((d) => d.id === session.directorId)
    : null;
  const activeTheme = currentDirector
    ? currentDirector.themeId === "custom" && currentDirector.customTheme
      ? currentDirector.customTheme
      : THEMES[currentDirector.themeId] || THEMES.cosmos
    : THEMES.cosmos;
    ...
    return (
      <ThemeContext.Provider value={activeTheme}>
        <DirectorAuth
          ...
          onLoginDirector={loginAsDirector}
          onRegister={registerDirector}
        />
      </ThemeContext.Provider>
    );
  }
      ...
      >
          ...
          <main className="flex-1 p-4 md:p-8 pb-28 md:pb-8 max-w-6xl mx-auto w-full">
            ...
            {view === "home" && (
              <DashboardHome
                ...
                directorData={directorData}
                opData={opData}
                centerLabel={director.centerName}
                allBranches={true}
              />
            )}
            {view === "branches" && (
            ...
            {view === "finance" && (
              <FinancePage
                role="director"
                scopeBranchIds={myBranches.map((b) => b.id)}
                directorData={directorData}
        ...
        {modal?.type === "branchForm" && (
          <BranchFormModal
            ...
            onSubmit={(p) =>
              modal.editing ? updateBranch(modal.editing.id, p) : addBranch(p)
            }
            onClose={closeModal}
          />
        )}
        {modal?.type === "branchDetail" &&
          (() => {
            const b = directorData.branches.find(
              (x) => x.id === modal.branchId,
            );
            return b ? (
              <BranchDetailModal
                branch={b}
        ...
        {modal?.type === "managerForm" && (
          <ManagerFormModal
            ...
            branches={myBranches}
            onSubmit={(p) =>
              modal.editing ? updateManager(modal.editing.id, p) : addManager(p)
            }
            onClose={closeModal}
          />
        )}
        {modal?.type === "managerPermissions" &&
          (() => {
            const mgr = directorData.managers.find(
              (m) => m.id === modal.managerId,
            );
            return mgr ? (
              <ManagerPermissionsModal
                manager={mgr}
                onSave={(allowed) => updateManagerPermissions(mgr.id, allowed)}
                onClose={closeModal}
              />
            ) : null;
          })()}
