...
const GLASS =
  "bg-slate-50 backdrop-blur-xl border border-slate-200 shadow-xl shadow-black/10";
const GLASS_SOFT = "bg-slate-50 backdrop-blur-lg border border-slate-200";
const INPUT_CLS =
  "w-full bg-slate-50 backdrop-blur-md border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-white/40 focus:bg-slate-100 transition-all text-sm";
const LABEL_CLS =
...
const BTN_GHOST =
  "bg-slate-50 hover:bg-slate-100 backdrop-blur-md border border-slate-200 text-slate-700 hover:text-slate-900 rounded-xl px-4 py-2 transition-all text-sm flex items-center justify-center gap-2";
const BTN_ICON =
  "w-9 h-9 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 flex items-center justify-center transition-all text-slate-600 hover:text-slate-900 shrink-0";
    ...
    >
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 bg-rose-500/80 hover:bg-rose-500 border border-slate-200 text-white rounded-xl px-4 py-2 text-sm transition-all"
          >
            Ha, tasdiqlash
          </button>
        </div>
...
function StatCard({ label, value, sub, icon: Icon, sticker }) {
  return (
    <div className={`${GLASS} rounded-2xl p-4 relative`}>
      {sticker && (
        <div
          ...
          style={{ background: sticker.bg || "#fff" }}
          title={sticker.title || ""}
          aria-hidden
        >
          {sticker.icon
            ? (() => {
                const Ic = sticker.icon;
                return <Ic size={16} className="text-white" />;
              })()
            : null}
        </div>
      )}
      ...
      {[1, 2, 3, 4, 5].map((s) => (
        <button key={s} type="button" onClick={() => onChange(s)}>
          <Star
            size={size}
            className={
              s <= value ? "fill-amber-300 text-amber-300" : "text-slate-400"
            }
          />
        </button>
      ))}
    ...
    <div className="flex flex-wrap gap-1.5">
      {WEEK_DAYS.map((d) => (
        <button
          ...
          type="button"
          onClick={() => toggle(d)}
          className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all ${value.includes(d) ? "bg-slate-100 border-slate-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-500"}`}
        >
          {d}
        </button>
      ))}
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <button
        ...
        className={`w-11 h-6 rounded-full transition-all relative shrink-0 ${checked ? "" : "bg-slate-50 border border-slate-200"}`}
          ...
          >
            {allThemes.map((t) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all ${theme.id === t.id ? "bg-slate-100 text-slate-900" : "text-slate-600 hover:bg-slate-50"}`}
              >
                <span
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          sticker={{ icon: TrendingUp, bg: theme.accent2, title: "Daromad" }}
        />
        <StatCard
          icon={CheckCircle2}
          label="Bu oy yig'ilgan"
          value={money(collected) + " so'm"}
          sticker={{ icon: Wallet, bg: theme.accent1, title: "Kirim" }}
        />
        <StatCard
          icon={TrendingDown}
          label="Bu oy xarajat"
              ...
              <div key={t.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                ...
                <div className="flex gap-2 text-[10px] text-slate-400">
                  {t.canCreateGroups === false && (
                    <span className="bg-slate-50 px-2 py-0.5 rounded-full">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 text-xs py-2 rounded-xl border transition-all ${approvalMode === "manager" ? "bg-slate-100 border-slate-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-500"}`}
              >
                Menejer ruhsati (darhol)
              </button>
              <button
                onClick={() => setApprovalMode("director")}
                className={`flex-1 text-xs py-2 rounded-xl border transition-all ${approvalMode === "director" ? "bg-slate-100 border-slate-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-500"}`}
              >
                ...
                >
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button
                      ...
                      className="w-9 h-9 rounded-xl bg-emerald-400/20 hover:bg-emerald-400/30 border border-emerald-300/40 flex items-center justify-center text-emerald-700"
                    >
                      <CheckCircle2 size={16} />
                    </button>
                    <button
                      onClick={() => rejectFinance(f.id)}
                      className="w-9 h-9 rounded-xl bg-rose-400/20 hover:bg-rose-400/30 border border-rose-300/40 flex items-center justify-center text-rose-700"
                    >
                      <XCircle size={16} />
                    </button>
                  </div>
            ...
            {history.map((f) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-slate-400 text-xs">{formatDate(f.date)}</p>
                </div>
                <p
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex gap-1.5 mt-1 flex-wrap">
                    {status === "paid" && (
                      <span className="text-[10px] bg-emerald-400/20 border border-emerald-300/40 text-emerald-700 px-2 py-0.5 rounded-full">
                        To'landi
                      </span>
                    )}
                    {status === "partial" && (
                      <span className="text-[10px] bg-amber-400/20 border border-amber-300/40 text-amber-700 px-2 py-0.5 rounded-full">
                        Qisman: {money(paidThis)}/{money(price)}
                      </span>
                    )}
                    {status === "unpaid" && (
                      <span className="text-[10px] bg-rose-400/20 border border-rose-300/40 text-rose-700 px-2 py-0.5 rounded-full">
                        To'lanmagan
                      </span>
                    )}
                    {hasDebt && (
                      <span className="text-[10px] bg-rose-500/30 border border-rose-400/50 text-rose-700 px-2 py-0.5 rounded-full">
                        O'tgan oydan qarz
                      </span>
                    )}
                    {overdue && (
          ...
          <div className="flex gap-2">
            {PAYMENT_METHODS.map((m) => (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 text-sm py-2.5 rounded-xl border transition-all ${method === m.id ? "bg-slate-100 border-slate-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-500"}`}
              >
                  ...
                  <div className="px-5 pb-5 border-t border-slate-200 pt-4 space-y-2">
                    ...
                    {canEdit && (
                      <button
                        ...
                        className={`${BTN_GHOST} w-full mt-2 text-rose-700 hover:text-rose-700`}
                      >
      ...
      >
        {items.map((item) => (
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all ${view === item.id ? "bg-slate-100 text-slate-900 shadow-lg" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
          >
    ...
    >
      {items.map((item) => (
        <button
          key={item.id}
          onClick={() => goTo(item.id)}
          className={`flex flex-col items-center gap-0.5 px-3 py-2 rounded-2xl transition-all shrink-0 ${view === item.id ? "bg-slate-100 text-slate-900" : "text-slate-500"}`}
        >
          <item.icon size={17} />
          ...
          {rooms.map((r) => {
            ...
            return (
              <div key={r.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                ...
                <span className="inline-block text-xs bg-emerald-400/15 border border-emerald-300/30 text-emerald-700 px-2.5 py-1 rounded-full">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-slate-50 px-2.5 py-1 rounded-full text-slate-600">
                      {pct}% davomat
                    </span>
                    <button
                      onClick={() =>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {ATTENDANCE_STATUSES.map((st) => (
                  <button
                    ...
                    className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all ${entries[s.id]?.status === st.id ? "bg-slate-100 border-slate-200 text-slate-900" : "bg-slate-50 border-slate-200 text-slate-400"}`}
                  >
                    {st.label}
                  </button>
                ))}
        ...
        <div className="flex gap-1.5 bg-slate-50 border border-slate-200 rounded-xl p-1">
          {[
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-lg transition-all ${filter === id ? "bg-slate-100 text-slate-900" : "text-slate-500"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="space-y-2">
          {filtered.map((n) => (
            <button
              ...
              className={`${GLASS} w-full text-left rounded-2xl p-4 flex items-start gap-3 transition-all ${n.read ? "opacity-60" : "hover:bg-slate-50"}`}
            >
              <span
                className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.read ? "bg-slate-100" : "bg-sky-400"}`}
              />
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`text-left rounded-xl border p-3 transition-all ${salaryType === "percent" ? "bg-slate-100 border-slate-200" : "bg-slate-50 border-slate-200"}`}
            >
              <p className="text-slate-900 text-sm font-medium">
                To'lovdan foiz
              </p>
            ...
            <button
              ...
              className={`text-left rounded-xl border p-3 transition-all ${salaryType === "fixed" ? "bg-slate-100 border-slate-200" : "bg-slate-50 border-slate-200"}`}
            >
              <p className="text-slate-900 text-sm font-medium">
                Belgilangan oylik
              </p>
        ...
        <div className="space-y-3">
          <div className="flex gap-1.5 bg-slate-50 border border-slate-200 rounded-xl p-1">
            <button
              ...
              className={`flex-1 text-xs py-2 rounded-lg transition-all ${tab === "advance" ? "bg-amber-400/30 text-amber-700" : "text-slate-500"}`}
            >
              Avans berish
            </button>
            <button
              onClick={() => setTab("salary")}
              className={`flex-1 text-xs py-2 rounded-lg transition-all ${tab === "salary" ? "bg-sky-400/30 text-sky-700" : "text-slate-500"}`}
            >
              Maosh to'lash
            </button>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {GROUP_COLORS.map((c) => (
              <button
                ...
                type="button"
                onClick={() => setColor(c)}
                className={`w-7 h-7 rounded-full border-2 ${color === c ? "border-white scale-110" : "border-slate-200"}`}
                style={{ background: c }}
              />
            ))}
          </div>
...
export default function App() {
      ...
      if (hasStorage) {
        try {
          const res = await window.storage
            .get(DIRECTOR_DATA_KEY, true)
            .catch(() => null);
          if (!cancelled)
            setDirectorData(
              res && res.value ? JSON.parse(res.value) : seedDirectorData(),
            );
        } catch (e) {
          if (!cancelled) setDirectorData(seedDirectorData());
        }
        try {
          const opRes = await window.storage
            .get(TEACHER_APP_KEY, true)
            .catch(() => null);
          if (!cancelled)
            setOpData(
              opRes && opRes.value
                ? JSON.parse(opRes.value)
                : { groups: [], students: [], tasks: [] },
            );
        } catch (e) {
          if (!cancelled) setOpData({ groups: [], students: [], tasks: [] });
        }
        try {
          const sessRes = await window.storage
            .get(MANAGER_SESSION_KEY, false)
            .catch(() => null);
          if (!cancelled)
            setSession(
              sessRes && sessRes.value ? JSON.parse(sessRes.value) : null,
            );
        } catch (e) {
          if (!cancelled) setSession(null);
        }
        try {
          const notifRes = await window.storage
            .get(MANAGER_NOTIF_KEY, false)
            .catch(() => null);
          if (!cancelled)
            setNotifLog(
              notifRes && notifRes.value ? JSON.parse(notifRes.value) : [],
            );
        } catch (e) {
          if (!cancelled) setNotifLog([]);
        }
      } else if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (!hasStorage) return;
      try {
        await window.storage.set(TEACHER_APP_KEY, JSON.stringify(opData), true);
      } catch (e) {
        console.error("Saqlashda xatolik:", e);
      }
    }, 700);
