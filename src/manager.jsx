...
const MANAGER_SESSION_KEY = 'manager-session-v1';
const MANAGER_NOTIF_KEY = 'manager-notif-log-v1';
...
function seedDirectorData() {
  return {
    ...
    directors: [
      { id: 'dir-test', name: 'Test Direktor', phone: '901234000', passwordHash: 'c837bd58a08f8ada06fb65e588af776b332e580dec1d042d75f32a6c89367297', centerName: 'Test Markaz', logo: null, address: '', themeId: 'cosmos', customTheme: null, twoFactorEnabled: false },
    ],
    branches: [
      { id: 'br-test', directorId: 'dir-test', name: 'Test filial', address: '', color: '#8b5cf6' },
    ],
    managers: [
      { id: 'mgr-test', branchIds: ['br-test'], name: 'Test Menejer', phone: '901234100', birthDate: '', address: '', passwordHash: '59b752c833bfedb868c0f7f308d9503709883550de40a41ff22bf484257dbb9e', monthlySalary: 0, rating: 0, allowedPages: MANAGER_NAV_ALL.map(p => p.id) },
    ],
    teachersHR: [],
    teacherPayments: [],
    holidays: [],
...
function Avatar({ name, color = '#8b5cf6', size = 40, photo, onClick }) {
  ...
  if (photo) return <img src={photo} alt={name} style={style} onClick={onClick} className={`rounded-full object-cover border-2 border-slate-200 ${onClick ? 'cursor-pointer' : ''}`} />;
  return (
    <div style={{ ...style, background: color, fontSize: size * 0.38 }} onClick={onClick} className={`font-display rounded-full flex items-center justify-center font-bold text-white border-2 border-slate-200 shrink-0 ${onClick ? 'cursor-pointer' : ''}`}>
      {initials(name)}
    </div>
  );
}
...
function PhoneInput({ value, onChange, autoFocus, onKeyDown }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-slate-600 text-sm shrink-0">+998</span>
      ...
      {open && (
        <>
          ...
          <div className={`${GLASS} rounded-2xl p-2 absolute right-0 top-11 z-50 w-72 max-h-80 overflow-y-auto`} style={{ background: '#ffffff' }}>
            ...
            {log.length === 0 ? <p className="text-slate-400 text-xs px-2 py-4 text-center">Hozircha bildirishnoma yo'q.</p> : log.map(n => (
              <div key={n.id} className="px-3 py-2 rounded-xl hover:bg-slate-50">
...
function opAttendance(opData) { return opData?.attendance || []; }
...
function getTeacherPayStats(directorData, opData, teacher, branch, month) {
  ...
  const expectedPay = teacher.salaryType === 'fixed'
    ? (teacher.fixedSalary || 0)
    : Math.round((branchStats?.expectedRevenue || 0) * (teacher.revenueSharePercent || 0) / 100);
  const payments = (directorData.teacherPayments || []).filter(p => p.teacherHRId === teacher.id && p.month === month);
  ...
  return { expectedPay, advances, salaryPaid, totalPaid, remaining: Math.max(0, expectedPay - totalPaid), payments };
}
...
function DashboardHome({ scopeBranches, directorData, opData, centerLabel, allBranches }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingCount > 0 && (
        <div className={`${GLASS} rounded-2xl p-4 flex items-center gap-3`}><Bell size={18} style={{ color: theme.accent1 }} /><p className="text-slate-900 text-sm flex-1">{pendingCount} ta xarajat tasdiqlashni kutmoqda.</p></div>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-4">Oylik kirim / chiqim (so'nggi 6 oy)</h3>
        ...
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-4">Xarajatlar turlari bo'yicha</h3>
          {pieData.length === 0 ? <p className="text-slate-400 text-sm py-10 text-center">Hali xarajat kiritilmagan.</p> : (
            <div style={{ width: '100%', height: 220 }}>
              <ResponsiveContainer>
                <PieChart>
        ...
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-4 flex items-center gap-2"><CreditCard size={16} /> To'lov turlari (bu oy)</h3>
          {paymentMethodData.length === 0 ? <p className="text-slate-400 text-sm py-10 text-center">Bu oy hali to'lov qabul qilinmagan.</p> : (
        ...
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-4 flex items-center gap-2"><GraduationCap size={16} /> O'qituvchilar bahosi</h3>
          {teacherRatingData.length === 0 ? <p className="text-slate-400 text-sm py-10 text-center">Hali o'qituvchi yo'q.</p> : (
            <div style={{ width: '100%', height: 200 }}>
              <ResponsiveContainer>
      ...
      {branchCompareData.length > 0 && (
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-4 flex items-center gap-2"><Building2 size={16} /> Filiallar bo'yicha sof foyda (bu oy)</h3>
          <div style={{ width: '100%', height: 220 }}>
            <ResponsiveContainer>
...
function TeachersHR({ scopeBranches, directorData, opData, openModal, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">O'qituvchilar</h2><p className="text-slate-500 text-sm mt-0.5">Ulush foizi, faol o'quvchilar va samaradorlik</p></div>
        {canEdit && <PrimaryButton onClick={() => openModal({ type: 'teacherHRForm' })}><Plus size={16} /> O'qituvchi qo'shish</PrimaryButton>}
      </div>

      {best && (
        <div className={`${GLASS} rounded-3xl p-5 flex items-center gap-3`}><span className="text-2xl">🏆</span><div><p className="text-slate-500 text-xs">Eng yuqori baholangan</p><p className="font-display text-slate-900 font-semibold">{best.name} — {best.rating}/5</p></div></div>
      )}
      ...
      {teachers.length === 0 ? (
        ...
        <div className="space-y-2">
          {teachers.map(t => {
            ...
            const stats = getTeacherPayStats(directorData, opData, t, branch, thisMonthKey());
            const activeStudents = opActiveStudents(opData);
            return (
...
function HolidaysPage({ directorId, directorData, addHoliday, removeHoliday, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div><h2 className="font-display text-2xl font-bold text-slate-900">Bayram va ta'til kunlari</h2><p className="text-slate-500 text-sm mt-0.5">Butun markaz uchun umumiy e'lon</p></div>
      {canEdit && (
...
function FinancePage({ role, scopeBranchIds, directorData, allBranches, addFinance, approveFinance, rejectFinance }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">Moliya</h2><p className="text-slate-500 text-sm mt-0.5">Xarajatlar va tasdiqlashlar (to'lovlar avtomatik kirim sifatida hisoblanadi)</p></div>
      ...
      {role === 'director' && pending.length > 0 && (
        <div className={`${GLASS} rounded-3xl p-5`}>
          <h3 className="font-display text-slate-900 font-semibold mb-3 flex items-center gap-2"><Bell size={16} /> Tasdiqlash kutilmoqda ({pending.length})</h3>
          <div className="space-y-2">
            {pending.map(f => {
...
function PaymentsPage({ scopeBranches, directorData, opData, openModal }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">To'lovlar</h2><p className="text-slate-500 text-sm mt-0.5">O'quvchilar to'lov holati va qarzdorlik</p></div>
...
function CoursesPage({ scopeBranches, directorData, opData, openModal, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">Kurslar</h2><p className="text-slate-500 text-sm mt-0.5">Kurs katalogi — har bir kurs ichida bir nechta guruh bo'lishi mumkin</p></div>
              ...
              <div key={c.id} className={`${GLASS} rounded-3xl overflow-hidden`}>
                <div className="w-full flex items-center justify-between p-5 flex-wrap gap-2">
                  <button onClick={() => setExpandedId(open ? null : c.id)} className="flex-1 text-left min-w-[200px]">
                    <p className="font-display text-slate-900 font-semibold">{c.name}</p>
                    <p className="text-slate-400 text-xs mt-0.5">{groups.length} guruh · {studs.length} o'quvchi</p>
                  </button>
                  <div className="flex items-center gap-2">
                    <div className="text-right"><p className="text-slate-900 font-semibold text-sm">{money(revenue)} so'm/oy</p></div>
                ...
                {open && (
                  <div className="px-5 pb-5 border-t border-slate-200 pt-4 space-y-2">
                    {groups.length === 0 ? <p className="text-slate-400 text-sm">Bu kursda hali guruh yo'q.</p> : groups.map(g => {
                      ...
                      return (
                        <div key={g.id} className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="text-slate-900 text-sm font-medium truncate">{g.name}</p>
                              <p className="text-slate-400 text-xs">{(g.days || []).join(', ') || 'Kunsiz'} · {g.time} · {g.durationMonths ? `${g.durationMonths} oy` : ''} · {gStuds.length} o'quvchi</p>
                            </div>
                            <div className="text-right shrink-0">
                              <p className="text-slate-900 text-sm font-semibold">{money(g.price || 0)} so'm/oy</p>
                              <p className="text-slate-400 text-[11px]">{paidCount}/{gStuds.length} to'liq to'lagan</p>
                            </div>
...
function GroupsPage({ directorData, opData, openModal, scopeBranchIds, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">Guruhlar</h2><p className="text-slate-500 text-sm mt-0.5">Barcha guruhlar va ularning oylik daromadi</p></div>
        ...
        <div className="space-y-2">
          {rows.map((g, i) => (
            <div key={g.id} className={`${GLASS} rounded-2xl p-4 flex items-center gap-3 flex-wrap`}>
              ...
              <div className="min-w-0 flex-1">
                <p className="text-slate-900 text-sm font-medium truncate">{g.name}</p>
                <p className="text-slate-400 text-xs truncate">{g.course?.name || 'Kurssiz'} · {(g.days || []).join(', ') || 'Kunsiz'} · {g.time} · {g.count} o'quvchi</p>
              </div>
...
function TopBar({ name, photo, color, now, onLogout, director, updateDirector, notifLog, onClearNotifs }) {
    ...
    <div className="space-y-3 mb-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div><p className="text-slate-500 text-xs">{dayName}, {now.getDate()}-{MONTHS_UZ[now.getMonth()]}, {now.getFullYear()}</p><p className="text-slate-900 font-medium mt-0.5">Xush kelibsiz, {name}! 👋</p></div>
        </div>
        <div className="flex items-center gap-2">
...
function RoomsPage({ opData, openModal, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">Xonalar</h2><p className="text-slate-500 text-sm mt-0.5">O'quv xonalarini boshqarish</p></div>
        {canEdit && <PrimaryButton onClick={() => openModal({ type: 'roomForm' })}><Plus size={16} /> Yangi xona qo'shish</PrimaryButton>}
      </div>
      <div className="grid grid-cols-3 gap-3">
        <StatCard icon={Building2} label="Jami xonalar" value={rooms.length} />
        <StatCard icon={Users} label="Katta xonalar (25+)" value={bigRooms} />
        <StatCard icon={Home} label="Umumiy sig'im" value={totalCapacity} />
      </div>
      {rooms.length === 0 ? (
        <EmptyState icon={Building2} title="Xonalar yo'q" subtitle="Birinchi xonani qo'shing" action={canEdit ? <PrimaryButton onClick={() => openModal({ type: 'roomForm' })}><Plus size={16} /> Xona qo'shish</PrimaryButton> : null} />
      ) : (
              ...
              <div key={r.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                ...
                {canEdit && (
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => openModal({ type: 'roomForm', editing: r })} className={`${BTN_GHOST} flex-1 justify-center`}><Pencil size={13} /> Tahrirlash</button>
                    <button onClick={() => openModal({ type: 'confirm', message: `"${r.name}" xonasini o'chirasizmi?`, action: { kind: 'deleteRoom', roomId: r.id } })} className={BTN_ICON}><Trash2 size={14} /></button>
                  </div>
                )}
              </div>
            );
...
function AttendancePage({ directorData, opData, scopeBranchIds, openModal }) {
  const [groupFilter, setGroupFilter] = useState('all');
  const courseIds = directorData.courses.filter(c => scopeBranchIds.includes(c.branchId)).map(c => c.id);
  const groups = opGroups(opData).filter(g => courseIds.includes(g.courseId));
  ...
  return (
    <div className="space-y-5">
      ...
      {records.length === 0 ? (
        <EmptyState icon={ClipboardList} title="Davomat yozuvlari yo'q" subtitle="O'qituvchi davomat olganda shu yerda ko'rinadi." />
      ) : (
        <div className="space-y-3">
          {records.map(rec => {
            ...
            return (
              <div key={rec.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
...
function EditAttendanceModal({ record, group, opData, onSave, onClose }) {
  ...
  return (
    <Modal title={`Davomatni tahrirlash — ${formatDate(record.date)}`} onClose={onClose}>
      <div className="space-y-3 max-h-[60vh] overflow-y-auto">
        {students.map(s => (
          <div key={s.id} className="space-y-2">
            ...
            {(entries[s.id]?.status === 'excused' || entries[s.id]?.status === 'late') && (
              <input value={entries[s.id]?.reason || ''} onChange={e => setReason(s.id, e.target.value)} placeholder="Sababi (ixtiyoriy)..." className={`${INPUT_CLS} text-xs py-2`} />
            )}
          </div>
        ))}
      </div>
      <PrimaryButton onClick={() => { onSave(entries); onClose(); }} className="w-full mt-4"><Check size={16} /> Saqlash</PrimaryButton>
    </Modal>
  );
}
...
function NotificationsPage({ notifLog, onMarkRead, onMarkAllRead, onClear }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-slate-900">Bildirishnomalar</h2><p className="text-slate-500 text-sm mt-0.5">Tizim xabarlari va e'lonlarni kuzating</p></div>
        <div className="flex gap-2">
...
function TeacherHRFormModal({ editing, branches, onSubmit, onClose }) {
  ...
  const [salaryType, setSalaryType] = useState(editing?.salaryType || 'percent');
  ...
  function submit() {
    ...
    onSubmit({ branchId, name: name.trim(), phone, salaryType, revenueSharePercent: parseFloat(sharePercent) || 0, fixedSalary: parseFloat(fixedSalary) || 0, rating, note: '', canCreateGroups, canReceivePayments });
    onClose();
  }
  ...
  return (
    <Modal title={editing ? "O'qituvchini tahrirlash" : "O'qituvchi qo'shish"} onClose={onClose}>
      <div className="space-y-4">
        ...
        {salaryType === 'percent' ? (
          <div><label className={LABEL_CLS}>Daromaddan ulush foizi (%)</label><input type="number" min="0" max="100" value={sharePercent} onChange={e => setSharePercent(e.target.value)} className={INPUT_CLS} /><p className="text-slate-400 text-[11px] mt-1">Oylik maosh filial daromadidan shu foiz asosida avtomatik hisoblanadi.</p></div>
        ) : (
          <div><label className={LABEL_CLS}>Oylik maosh (so'm)</label><input type="number" min="0" value={fixedSalary} onChange={e => setFixedSalary(e.target.value)} className={INPUT_CLS} /><p className="text-slate-400 text-[11px] mt-1">Har oy to'lovlar hajmidan qat'i nazar belgilangan summa.</p></div>
        )}
...
function TeacherPayrollModal({ teacher, branch, directorData, opData, onAddPayment, onClose }) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (hasStorage) {
        ...
        try {
          const sessRes = await window.storage.get(MANAGER_SESSION_KEY, false).catch(() => null);
          if (!cancelled) setSession(sessRes && sessRes.value ? JSON.parse(sessRes.value) : null);
        } catch (e) { if (!cancelled) setSession(null); }
        try {
          const notifRes = await window.storage.get(MANAGER_NOTIF_KEY, false).catch(() => null);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!hasStorage) return;
      try {
        if (session) await window.storage.set(MANAGER_SESSION_KEY, JSON.stringify(session), false);
        else await window.storage.delete(MANAGER_SESSION_KEY, false);
      } catch (e) { /* ignore */ }
    })();
  }, [session, loading]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (!hasStorage) return;
      try { await window.storage.set(MANAGER_NOTIF_KEY, JSON.stringify(notifLog), false); }
      catch (e) { /* ignore */ }
    }, 700);
    return () => clearTimeout(t);
  }, [notifLog, loading]);
  ...
  function loginAsManager(managerId) { setSession({ managerId }); setView('home'); }
  ...
  function addTeacherPayment(payload) {
    setDirectorData(prev => ({ ...prev, teacherPayments: [...(prev.teacherPayments || []), { id: generateId('tpay'), createdAt: Date.now(), ...payload }] }));
    addNotification(payload.type === 'advance' ? `Avans berildi: ${money(payload.amount)} so'm.` : `Maosh to'landi: ${money(payload.amount)} so'm.`);
  }
  ...
  function addRoom(payload) {
    setOpData(prev => ({ ...prev, rooms: [...(prev.rooms || []), { id: generateId('room'), ...payload }] }));
    addNotification("Xona qo'shildi.");
  }
  function updateRoom(id, payload) {
    setOpData(prev => ({ ...prev, rooms: (prev.rooms || []).map(r => r.id === id ? { ...r, ...payload } : r) }));
  ...
  function deleteRoom(id) {
    setOpData(prev => ({
      ...
      groups: (prev.groups || []).map(g => g.roomId === id ? { ...g, roomId: null } : g),
    }));
    addNotification("Xona o'chirildi.");
  }

  function updateAttendanceRecord(recordId, entries) {
    setOpData(prev => ({ ...prev, attendance: (prev.attendance || []).map(a => a.id === recordId ? { ...a, records: entries, locked: true } : a) }));
    addNotification('Davomat yangilandi.');
  }
  function deleteAttendanceRecord(recordId) {
    setOpData(prev => ({ ...prev, attendance: (prev.attendance || []).filter(a => a.id !== recordId) }));
  ...
  function handleConfirm() {
    ...
    if (action.kind === 'deleteAttendance') deleteAttendanceRecord(action.recordId);
    setModal(null);
  }

  if (loading || !directorData || !opData) return <ThemeContext.Provider value={THEMES.cosmos}><LoadingScreen /></ThemeContext.Provider>;

  const currentManager = session ? directorData.managers.find(m => m.id === session.managerId) : null;
  const orgDirectorId = currentManager ? directorData.branches.find(b => (currentManager.branchIds || []).includes(b.id))?.directorId : null;
  const orgDirector = orgDirectorId ? directorData.directors.find(d => d.id === orgDirectorId) : null;
  const activeTheme = orgDirector ? (orgDirector.themeId === 'custom' && orgDirector.customTheme ? orgDirector.customTheme : (THEMES[orgDirector.themeId] || THEMES.cosmos)) : THEMES.cosmos;

  if (!session) {
    return <ThemeContext.Provider value={THEMES.cosmos}><ManagerAuth directorData={directorData} onLoginManager={loginAsManager} /></ThemeContext.Provider>;
  }
  ...
  if (!manager || myBranches.length === 0) return <ThemeContext.Provider value={THEMES.cosmos}><LoadingScreen /></ThemeContext.Provider>;
      ...
      <div className="min-h-screen w-full text-slate-900 relative" style={{ ...PAGE_BG_STYLE, fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
        ...
        <div className="relative z-10 flex min-h-screen">
          ...
          <main className="flex-1 p-4 md:p-8 pb-28 md:pb-8 max-w-6xl mx-auto w-full">
            <TopBar name={manager.name} color={myBranches[0].color} now={now} onLogout={logout} director={orgDirector} updateDirector={updateDirector} notifLog={notifLog} onClearNotifs={clearNotifLog} />
            ...
            {effectiveView === 'groups' && <GroupsPage directorData={directorData} opData={opData} openModal={openModal} scopeBranchIds={branchIds} canEdit={true} />}
            {effectiveView === 'attendance' && <AttendancePage directorData={directorData} opData={opData} scopeBranchIds={branchIds} openModal={openModal} />}
            {effectiveView === 'rooms' && <RoomsPage opData={opData} openModal={openModal} canEdit={true} />}
            ...
            {effectiveView === 'holidays' && <HolidaysPage directorId={orgDirectorId} directorData={directorData} addHoliday={addHoliday} removeHoliday={removeHoliday} canEdit={false} />}
            {effectiveView === 'notifications' && <NotificationsPage notifLog={notifLog} onMarkRead={markNotifRead} onMarkAllRead={markAllNotifsRead} onClear={clearNotifLog} />}
          </main>
        </div>
        ...
        {modal?.type === 'teacherPayroll' && (() => { const t = directorData.teachersHR.find(x => x.id === modal.teacherId); const b = t ? directorData.branches.find(x => x.id === t.branchId) : null; return t ? <TeacherPayrollModal teacher={t} branch={b} directorData={directorData} opData={opData} onAddPayment={addTeacherPayment} onClose={closeModal} /> : null; })()}
        ...
        {modal?.type === 'roomForm' && <RoomFormModal editing={modal.editing} onSubmit={p => modal.editing ? updateRoom(modal.editing.id, p) : addRoom(p)} onClose={closeModal} />}
        {modal?.type === 'editAttendance' && (() => { const rec = opAttendance(opData).find(a => a.id === modal.recordId); const grp = opGroups(opData).find(g => g.id === modal.groupId); return rec && grp ? <EditAttendanceModal record={rec} group={grp} opData={opData} onSave={entries => updateAttendanceRecord(rec.id, entries)} onClose={closeModal} /> : null; })()}
