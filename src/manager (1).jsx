  ...
  return (
    <div className="min-h-screen w-full text-white relative flex" style={{ background: theme.bg, fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-center px-16 relative z-10">
        ...
        <p className="text-white/60 text-base max-w-md">O'quvchi qabuli, guruhlar, to'lovlar va moliya so'rovlari — direktor sizga biriktirgan filial(lar) doirasida.</p>
      </div>
      <div className="flex-1 flex items-center justify-center p-4 relative z-10">
        <div className={`${GLASS} rounded-3xl p-6 sm:p-8 w-full max-w-sm`} style={{ background: 'rgba(10,20,60,0.85)' }}>
          ...
          <div className="space-y-3">
            ...
            <p className="text-white/30 text-[11px] text-center pt-1">Hisobingiz yo'qmi? Direktoringizdan menejer sifatida qo'shilishingizni so'rang.</p>
          </div>
        </div>
      </div>
    </div>
...
function RoomsPage({ opData, openModal, canEdit }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display text-2xl font-bold text-white">Xonalar</h2><p className="text-white/50 text-sm mt-0.5">O'quv xonalarini boshqarish</p></div>
            ...
            return (
              <div key={r.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                <div className="flex items-start gap-3">
                  <span className="w-10 h-10 rounded-xl bg-white/10 grid place-items-center shrink-0"><Home size={18} /></span>
                  <div className="min-w-0 flex-1">
                    <p className="text-white font-medium truncate">{r.name}</p>
                    <p className="text-white/40 text-xs">O'quv xonasi</p>
                  </div>
                </div>
                <span className="inline-block text-xs bg-emerald-400/15 border border-emerald-300/30 text-emerald-200 px-2.5 py-1 rounded-full"><Users size={11} className="inline -mt-0.5 mr-1" />{r.capacity || 0} ta sig'im</span>
                {usedByGroups.length > 0 && <p className="text-white/35 text-[11px]">{usedByGroups.length} ta guruhda band</p>}
                {canEdit && (
...
function AttendancePage({ directorData, opData, scopeBranchIds, openModal }) {
  ...
  return (
    <div className="space-y-5">
      <div><h2 className="font-display text-2xl font-bold text-white">Davomat</h2><p className="text-white/50 text-sm mt-0.5">Barcha guruhlar bo'yicha davomat yozuvlari</p></div>
              ...
              <div key={rec.id} className={`${GLASS} rounded-2xl p-4 space-y-3`}>
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    ...
                    <p className="text-white/40 text-xs">{formatDate(rec.date)} {rec.locked && <span className="ml-1.5 text-emerald-300">· qulflangan</span>}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full text-white/70">{pct}% davomat</span>
                ...
                <div className="flex gap-3 flex-wrap text-xs text-white/60">
                  ...
                  {students.length - marked > 0 && <span className="text-white/30">Belgilanmagan: {students.length - marked}</span>}
                </div>
              </div>
            );
          })}
        ...
        {students.map(s => (
          <div key={s.id} className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <p className="text-white text-sm flex-1 truncate">{s.name}</p>
              <div className="flex gap-1.5 flex-wrap">
                {ATTENDANCE_STATUSES.map(st => (
                  <button key={st.id} onClick={() => setStatus(s.id, st.id)} className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all ${entries[s.id]?.status === st.id ? 'bg-white/25 border-white/40 text-white' : 'bg-white/5 border-white/10 text-white/40'}`}>{st.label}</button>
                ))}
              </div>
            </div>
